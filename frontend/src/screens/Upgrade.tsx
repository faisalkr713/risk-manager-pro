...
const Upgrade: React.FC = () => {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto' }}>
      ...
      {!isPro && (
        <div style={{ background: '#1E1E1E', border: '1px solid #2A2A2A', borderRadius: 16, padding: 24, marginBottom: 28 }}>
          <h3 style={{ color: '#fff', margin: '0 0 16px', fontSize: 16, fontWeight: 700 }}>This Month's Usage</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 32 }}>
        {/* Free */}
        <div style={{ background: '#1E1E1E', border: '1px solid #2A2A2A', borderRadius: 16, padding: 28 }}>
          <div style={{ color: '#888', fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Free</div>
          <div style={{ color: '#fff', fontSize: 36, fontWeight: 800, marginBottom: 4 }}>$0</div>
      ...
      {/* Buy Me a Coffee */}
      <div style={{ background: '#1E1E1E', border: '1px solid #2A2A2A', borderRadius: 16, padding: 28, display: 'flex', gap: 32, alignItems: 'center' }}>
        <div style={{ flexShrink: 0 }}>
          {qrUrl ? (
            <img src={qrUrl} alt="Donation QR" style={{ width: 160, height: 160, borderRadius: 12, display: 'block', background: '#fff', padding: 4 }} />
          ) : (
            <div style={{ width: 160, height: 160, background: '#2A2A2A', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#555' }}>QR Loading...</div>
          )}
          <div style={{ textAlign: 'center', color: '#555', fontSize: 11, marginTop: 8 }}>Scan to donate</div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 28, marginBottom: 8 }}>☕</div>
          <h3 style={{ color: '#fff', margin: '0 0 8px', fontSize: 20, fontWeight: 700 }}>Buy Me a Coffee</h3>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <span style={{ color: isMax ? '#FF1744' : isNear ? '#FFD600' : '#fff', fontSize: 13, fontWeight: 700 }}>
          {used}/{limit}
        </span>
      </div>
...
const FeatureItem: React.FC<{ text: string; included: boolean; pro?: boolean }> = ({ text, included, pro }) => (
  <li style={{ display: 'flex', alignItems: 'center', gap: 10, color: included ? (pro ? '#fff' : '#B0B0B0') : '#444', fontSize: 14 }}>
